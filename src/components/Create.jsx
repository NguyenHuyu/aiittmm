...
const Create = () => {
  ...
  return (
    <div className='w-full min-h-screen flex items-center justify-center'>
      <div className='w-[90%] md:w-[75%] border border-red-900 rounded-md p-4 flex flex-col items-center justify-center md:gap-4'>
        {
          fields && (
            <motion.p initial={{opacity:0}}
                      animate={{opacity:1}}
                      exit={{opacity:0}}
                      className={`w-full p-2 rounded-sm text-center text-sm font-semibold ${alert === 'danger' 
             ? 'bg-red-400 text-red-800' 
             : 'bg-emerald-400 text-orange-900' }`}>
              {msg}
            </motion.p>
          )}
          <div className="md:w-full md:py-2 border-b border-gray-300 md:flex md:items-center md:gap-2">
          ...
          {/* <div className='w-full'>
            <select onChange={(e)=> setCategory(e.target.value)}
                    className='outline-none w-full md:text-base border-b-2 border-gray-200 md:p-2 rounded-md cursor-pointer' >
              <option value="other"
                      className='bg-white'>
                      Selection 
              </option>
              {Category && Category.map((item)=>(
                <option key={item.id}
                        className='md:text-base border-0 outline-none capitalize bg-white md:text-headingColor'
          ...
          <div className='md:group md:flex md:justify-center md:items-center 
                ...
                { isLoading ? <Loader/> : (<>
                  {!image ? <>
                    <label className='md:w-full md:h-full md:flex md:flex-col md:items-center md:justify-center cursor-pointer'>
                      <div className='md:w-full md:h-full md:flex md:flex-col md:items-center md:justify-center cursor-pointer md:gap-2'>
                        <MdCloudUpload className='md:text-gray-500 md:text-3xl hover:text-gray-700'/>
                        <p className='md:text-gray-500 hover:text-gray-700'>Click to Upload</p>
                      </div>
                      <input type="file"
                            name='uploadimage'
                            accept='image/*'
